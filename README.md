...
```bash
GET /api/usuario-profile/{userId}
Authorization: Bearer {token_admin}
```
...
#### Paso 2: Subir imagen al Pelicula
...
- La URL debe empezar con `http://localhost:3010/uploads/`
...
```bash
curl -X PUT http://localhost:3010/api/usuario-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
